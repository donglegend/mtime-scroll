;(function() {
        ...
        me.momentum = function(
            ...
            if (destination < lowerMargin) {
                destination = wrapperSize
                    ? lowerMargin -
                      Math.min(wrapperSize / 2.5 * (speed / 8), wrapperSize / 3)
                    : lowerMargin
    ...
    var MScroll = (function() {
        function MScroll(el, options) {
            ...
            // 用来控制 end事件是否需要阻止默认，子元素绑定了 touchend会 在滑动结束触发，不合理
            this.isMove = false
    ...
    MScroll.prototype = {
        ...
        _move: function(e) {
            ...
            if (newX > 0 || newX < this.maxScrollX) {
                newX = this.options.bounce
                    ? this.x + deltaX / this.options.dragForce
                    : newX > 0 ? 0 : this.maxScrollX
            }
            if (newY > this.minScrollY || newY < this.maxScrollY) {
                newY = this.options.bounce
                    ? this.y + deltaY / this.options.dragForce
                    : newY > this.minScrollY ? this.minScrollY : this.maxScrollY
            }
        ...
        refresh: function() {
            ...
            this.hasVerticalScroll =
                this.options.scrollY && this.maxScrollY < this.options.topOffset
        ...
        _initEvents: function(remove) {
            ...
            if (!this.options.disableMouse) {
                ...
                eventType(target, 'mouseup', this, true)
            }
            ...
            if (!this.options.disableTouch) {
                ...
                eventType(target, 'touchend', this, true)
            }
    ...
    var Indicator = (function() {
        ...
        Indicator.createDefaultScrollbar = function(direction) {
            ...
            indicator.style.cssText =
                '-webkit-box-sizing:border-box;-moz-box-sizing:border-box;box-sizing:border-box;position:absolute;background:rgba(0,0,0,0.4);border-radius:4px;'

            if (direction == 'v') {
                scrollbar.className = 'MScrollVerticalBar'
                scrollbar.style.cssText +=
                    ';width: 5px;bottom: 2px;top: 2px;right: 1px;overflow: hidden'
                indicator.style.cssText += ';width: 100%;'
            }
            if (direction == 'h') {
                scrollbar.className = 'MScrollHorizontalBar'
                scrollbar.style.cssText +=
                    ';height: 5px;bottom: 0px;left: 2px;right: 2px;overflow: hidden'
    ...
    Indicator.prototype = {
        ...
        refresh: function() {
            ...
            this.indicatorHeight =
                this.wrapperHeight *
                this.wrapperHeight /
                (this.scroller.scrollerHeight + this.topOffset ||
                    this.wrapperHeight ||
                    1)
            this.indicatorStyle.height = this.indicatorHeight + 'px'
            // 计算 相对比例，相对于与滑动容器缩放比
            this.maxPosY = this.wrapperHeight - this.indicatorHeight
            this.sizeRatioY = Math.min(
                this.maxPosY / (this.scroller.maxScrollY - this.topOffset || 1),
                1
            )
            this.updatePosition()
            this.fade()
        ...
        updatePosition: function() {
            var x = Math.round(this.sizeRatioX * this.scroller.x) || 0,
                y =
                    Math.round(
                        this.sizeRatioY * (this.scroller.y - this.topOffset)
                    ) || 0
